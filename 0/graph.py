# A module to handle the communication between the MUSE and PC, computing
# the spectrogram of one channel, and sending the results to a ZMQ client,
# in real-timeself.
# clone of https://github.com/sagarpatel/BCI_Experiments/blob/master/connectMuse.py
...
class connectMuse:
    """
    Class that handles:
        1) the communication between muse-io and python using pythonosc packets
        2) the computation of the spectrogram
        3) the display in matplotlib of the raw EEG and accelerometers signals,
            and EEG spectrogram
        ...
        muse-io has to be started first in a console:
            ...
            OR
            muse-io --preset 14 --device 00:06:66:65:92:94 --osc osc.udp://localhost:4000 --osc-timestamp

        Example useage:
            plotData = True -> Choose between plotting data or just computing
                    the spectrogram
    ...
    def __init__(self, bufferSize=10, spectrUpdatePeriod=1):
        ...
        self.Fs = 220 # For more robustness, the Fs and accFs should be taken
                        # from the OSC packet /muse/config
        self.accFs = 50
